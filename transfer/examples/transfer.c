...
int trans_string(redisInfo *src, redisInfo *dst, char * keyname) {
	...
	snprintf(cmd, 100, "ttl %s", keyname);
	reply = redisCommand(src->rd, cmd);
	if (!reply) {
	...
	if (reply->type == REDIS_REPLY_STRING) {
		...
		reply_set = redisRestoreCommand (dst->rd, keyname, 0,reply->str, reply->len);
...
//	redisContext *dst;
	...
	status = conn_redis(&src, src_host, src_port);
	...
	status = conn_redis(&dst, dst_host, dst_port);
	if (status != REDIS_OK) {
